...
SC_MODULE(router){
	...
	void map_rtg(){
		...
		if(bfE->dout.caminho.size() == 0){
			...
			rtgE.destiny = bfE->din.caminho.front();
		...
		if(bfS->dout.caminho.size() == 0){
			...
			rtgS.destiny = bfS->din.caminho.front();
		...
		if(bfW->dout.caminho.size() == 0){
			...
			rtgW.destiny = bfW->din.caminho.front();
			portDestiny[3] = rtgW.tableAcess();
		}

		if(bfL->dout.caminho.empty() == 1){
			portDestiny[4] = LOCAL;
		}else{
			rtgL.destiny = bfL->din.caminho.front();
